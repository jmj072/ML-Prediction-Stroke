...
### 개선사항
  - 다른 Over Sampling 방법을 시도해볼 것
    - 사전에 사용한 SMOTE 방법 외에, Borderline-SMOTE, ADASYN과 같은 방법들을 추가로 알아보고 적용해볼것
    - 데이터가 손실되는 Down Sampling 방법보다는 Over Sampling 방법을 최대한 이용해볼것
  - Down Sampling 시도
    - 정상군의 수가 20여 만명, 뇌졸중 환자가 1만 여명 이므로, 뇌졸중 환자 수에 맞추어 정상군의 수를 조정하였어도 예측 모델 생성이 가능했을 것으로 생각됨

  - 연령대 수정
    - 연령대 그룹을 20대, 30대와 같이 10년 단위로 변경할 것
...
- 데이터 불균형 문제를 해결하려 하였으나, 제대로 해결하지 못해 아쉽다.
  ...
  - 프로젝트 당시에 손실될 데이터가 아까워서 Down sampling을 시도해보진 않았는데, 뇌졸중 환자 수에 맞추어도 약 2만여개의 데이터가 남아있으므로 모델을 형성하기엔 충분한 데이터였을 것 같다.
...
*프로젝트의 자세한 사항은 다음 [PPT 자료](https://drive.google.com/file/d/1h63JIlSbAJNnv0xOYYF4DaSQBdq3t2BP/view?usp=sharing)를 확인해주세요*
